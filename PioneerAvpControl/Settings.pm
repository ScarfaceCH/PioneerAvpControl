package Plugins::PioneerAvpControl::Settings;
...
# ----------------------------------------------------------------------------
my $prefs = preferences('plugin.pioneeravpcontrol'); #name of preferences
...
sub new {
	...
	$log->debug( "*** PioneerAvpControl::Settings::new() " . $classPlugin . "\n");
...
sub name { #this is what is shown in the players menu on the web gui
	return 'PLUGIN_PIONEERAVPCONTROL_MODULE_NAME';
}
...
sub page { #tells which file to use as the web page
	return 'plugins/PioneerAvpControl/settings/basic.html';
}
...
sub handler {
	...
	if( !defined( $client)) {
		...
		$log->debug( "*** PioneerAvpControl: found player: " . $client . "\n");
	}
	...
	if( !$params->{'playername'}) {
		...
		$log->debug( "*** PioneerAvpControl: player name: " . $params->{'playername'} . "\n");
	}
	...
	if ($params->{'saveSettings'}) {
		...
		$prefs->client($client)->set('input', "$params->{'input'}");
	...
	# set the input setting on the web page	
	$params->{'prefs'}->{'input'} = $prefs->client($client)->get('input'); 
